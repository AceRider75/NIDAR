...
class Controller:
    ...
    def _telemetry_loop(self):      #Telemetry thread to read data from PixHawk
        while self.running:
            try:
                ...
                log_message("PixHawk", f"Telemetry error: {e}\n")
                time.sleep(0.5)
                # Try to reconnect
                try:
                    self.the_connection.close()
                    time.sleep(1)
                    self.the_connection = mavutil.mavlink_connection('/dev/ttyACM0')
                    self.the_connection.wait_heartbeat(timeout=5)
                    log_message("PixHawk", "Reconnected to PixHawk\n")
                    self.the_connection.mav.request_data_stream_send(
                        self.the_connection.target_system,
                        self.the_connection.target_component,
                        mavutil.mavlink.MAV_DATA_STREAM_ALL,
                    ...
                    log_message("PixHawk", f"Reconnection failed: {reconnect_err}\n")
                    time.sleep(2)
            
            # Log telemetry periodically (every 0.1s to reduce I/O)
    ...
    def _control_loop(self):
            ...
            while self.running:
                ...
                # 3. Target Reached Logic (Simplified)
                # We only enter this if we haven't processed this specific target index yet
                elif d_h <= HORIZ_TOL and d_v <= VERT_TOL and count > 0 and count > last_processed_count:
                    print(f"Reached Target {count}")
                    last_processed_count = count # Mark this index as done
