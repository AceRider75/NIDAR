...
import numpy as np
import os, sys
BASE_DIR = os.path.dirname(os.path.dirname(__file__))  # .../yellow_detector.py
...
class LargestSpotTracker:
    ...
    def process(self, frame):
        ...
        if cnt is not None:
            ...
            # Draw overlays (largest spot only) - replace rectangle with smooth curve
            ...
            # Keep last known position (no new detection)
...
def main():
    ...
    while True:
        ...
        if not ok:
            break
        frame_out, mask, info = tracker.process(frame)

        cv2.imshow("Largest Yellow Spot (merged detector+tracker)", frame_out)
